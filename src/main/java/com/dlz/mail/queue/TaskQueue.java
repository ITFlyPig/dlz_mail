...
public class TaskQueue {
    ...
    public synchronized void addSqlTask(List<MailTaskBean> tasks) {
        if (tasks == null || tasks.size() == 0) {
            Log.d("要添加的任务列表为空，线程进入阻塞");
            try {
                wait();
            } catch (InterruptedException e) {
                e.printStackTrace();
                Log.d("要添加的任务列表为空，线程进入阻塞  阻塞异常：" + e.getLocalizedMessage());
            }
            return;
        }
        ...
        try {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (bean.getExcuteTime() == null || bean.getExcuteTime().getTime() <= System.currentTimeMillis()) {
                    ...
                    Log.d("任务：" + bean.getTask_name() + " 立即执行");
                } else {
                    ...
                    Log.d("任务：" + bean.getTask_name() + " 暂时不执行，将其定时");
    ...
    public void removeOldTimers( List<MailTaskBean> tasks){
        ...
        for (MailTaskBean mailTaskBean:tasks){
            if (TextUtil.isEmpty(mailTaskBean.getTask_name())){
                Log.d(mailTaskBean.getId() + "任务名称为空，跳过删除定时器的步骤");
                continue;
            }
            Log.d("删除定时器：" + mailTaskBean.getTask_name());
            QuartzManager.removeJob(mailTaskBean.getTask_name() + "邮件发送", "send_email");
            QuartzManager.removeJob(mailTaskBean.getTask_name() + "sql查询", "excute_sql");
